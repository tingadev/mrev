...
if(isset($_GET['id']))
  global $connection;
  $id = $_GET['id'];
  $query = "SELECT *from services a, services_desc ad where a.id = ad.aid and a.id = $id and lang ='".$lang."'";
  ...
  $root = "../uploads/services/$src_img_thumb";
...
if(isset($_POST['updateServices'])){
    ...
    if($flg==0){
      move_uploaded_file($photo_up_temp,ROOT_SRC."/uploads/services/$photo_up");
      $info= pathinfo($photo_up);
      $file_name =  basename($photo_up,'.'.$info['extension']);
      $thumb_up= $file_name . "_thumb." . strtolower($info['extension']);
      make_thumb(ROOT_SRC."/uploads/services/$photo_up",ROOT_SRC."/uploads/services/$thumb_up",700);
      $data['thumb'] =$thumb_up;
    }
    // SEO URL
    ...
    $res = update('services_desc',$data,"lang` ='".$lang."' and `aid",$id);
    $res_seo=update('seo_url',$seo_url,"lang` ='".$lang."' and modules = '".$modules."' and `itemid",$edit_id);
    if($res['mysqli_error']){
...
?>
  ...
  <div class="card-body">
    ...
    <form action='' method="post" enctype="multipart/form-data" target="_self">
      ...
      <input type="submit" name="updateServices" class="btn btn-primary" value="Submit">
    </form>
  </div>
</div>
    </div>
